function fig00_1(output::Symbol, font::String, scale::Float64)
  ...
  """
      juliaLuxor.circles(radius=100)
  
  Draw the three Julia Luxor.circles in color centered at the origin.
  
  The distance of the centers of the Luxor.circles from the origin is `radius`.
  The optional keyword arguments `outercircleratio` (default 0.75) and `innercircleratio`
  (default 0.65) control the radius of the individual colored Luxor.circles relative to the `radius`.
  So you can get relatively smaller or larger Luxor.circles by adjusting the ratios.
  """
  function juliacircles(radius=100; outercircleratio=0.75, innercircleratio=0.65)
      ...
      for (n, p) in enumerate(points)
          ...
          Luxor.circle(p, outercircleratio * radius, :fill)
          setcolor(color_sequence[n][2]...)
          Luxor.circle(p, innercircleratio * radius, :fill)
      end
  end